...
- [ ] **Paso 1: Centralización del Gestor de Estado (`shared/config_manager.py`)**
  - [ ] Crear carpeta `shared/` y módulo `config_manager.py`.
  - [ ] Implementar `load_json` y `save_json` seguros.
  - [ ] Aplicar `filelock` con timeout solo para archivos de configuración compartidos (`config.json`, `outputs.json`). No aplicar a los archivos de memoria de la IA (ya protegidos por el worker pattern del bot).
  - [ ] Reemplazar llamadas locales duplicadas en: `launcher.py`, `meowSick.py`, `music.py`, `memory.py`, `evolution.py` y `utils.py`.
  - [ ] Verificar que `_get_config()` en `AIManager` (Singleton) no cachee el resultado para que el hot-reload de Cogs funcione correctamente.

- [ ] **Paso 2: Extracción de Temas y Estilos Visuales (`themes/dark.json`)**
  - [ ] Crear carpeta `themes/` y archivo `dark.json` con la paleta de colores.
  - [ ] Crear clase `ThemeManager` en `shared/theme_manager.py`.

- [ ] **Paso 3: Sistema Base de Idiomas (`settings/locales/`)**
  - [ ] Crear carpeta `settings/locales/` y extraer todos los strings de la UI a `es.json`.
  - [ ] Crear archivo `en.json` (traducción base).
  - [ ] Crear clase `LanguageManager` en `shared/language_manager.py`.
